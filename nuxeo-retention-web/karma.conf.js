...
const reporters = coverage ? ['mocha', 'coverage-istanbul'] : ['mocha', 'karmaHTML'];
...
module.exports = (config) => {
  ...
  config.set({
    ...
    files: [
      // Include JavaScript test files
      /* { pattern: `test/*${config.grep || '*.test.js'}`,
      type: 'module' }, */

      // Include HTML files with embedded JavaScript
      //  { pattern: 'document/retentionrule/*.html', included: false, served: true, watched: true, type: 'module' },
      { pattern: 'test/*.js', watched: true, served: true, included: true, type: 'module' },
    ],
    plugins: [
      // load plugin
    ...
    client: {
      ...
      karmaHTML: {
        source: [{ src: 'document/retentionrule/nuxeo-retentionrule-create-layout.html', tag: 'index' }],
      },
    },
  });
};
